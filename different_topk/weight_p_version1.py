...
class TopKPooling(torch.nn.Module):
    ...
    def __init__(self, in_channels, ratio=0.5, min_score=None, multiplier=1,
        ...
        self.weight = Parameter(torch.Tensor(2, in_channels))
		self.weightp = Parameter(torch.Tensor(1))

        self.reset_parameters()

    def reset_parameters(self):
        size = self.in_channels
        uniform(size, self.weight[0])
		uniform(size, self.weight[1])
    ...
    def forward(self, x, edge_index, edge_attr=None, batch=None, attn=None):
        ...
        score = ((attn * self.weight[0]).sum(dim=-1)+self.weightp*(attn * self.weight[1]).sum(dim=-1))/(self.weightp+1)
